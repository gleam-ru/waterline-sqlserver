...
module.exports = (function () {
  ...
  var adapter = {
    ...
    defaults: {
      ...
      persistent: process.env.MSSQL_PERSISTENT||true,
    ...
    connectConnection: function(conn, cb) {
      ...
      if (!persistent || !connection.uniqId) {
        connection.uniqId = _.uniqueId();
      }
      if (!connection.mssqlConnection) {
        connection.mssqlConnection = new mssql.Connection(marshalConfig(connection.config), function(err) {
          if (connection) {
      ...
      else if (!connection.mssqlConnection[connection.uniqId]) {
        connection.mssqlConnection[connection.uniqId] = new mssql.Connection(marshalConfig(connection.config), function(err) {
          if (connection) {
            connection.persistent = persistent;
          }
          return cb(err, connection.uniqId);
        });
      }
      else {
        return cb(null, connection.uniqId);
      }
    },
    ...
    find: function (connection, collection, options, cb) {
      ...
      adapter.connectConnection(connection, function __FIND__(err, uniqId) {
        ...
        request.query(statement, function (err, recordset) {
          if (err) {
            console.error('SQL error! Statement:', statement);
            return cb(err);
          }
